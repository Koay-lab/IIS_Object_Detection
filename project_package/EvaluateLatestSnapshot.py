...
def parse_test_txt_file(test_file, image_dir, annotation_dir):
    dataset = []
    ...
    with open(test_file, 'r') as lfile:
        ...
        for each_line in lines:
            ...
            if image_dir is None:
                abs_image_path = os.path.join(path_to_dataset, image)
            else:
                abs_image_path = os.path.join(image_dir, os.path.basename(image))

            if annotation_dir is None:
                abs_anno_path = os.path.join(path_to_dataset, anno)
            else:
                abs_anno_path = os.path.join(annotation_dir, os.path.basename(anno))
...
def deal_with_xmls(annotation_dir: str, output_dir: str):
    annotations = glob.glob(os.path.join(annotation_dir, "*xml"))
...
class DetectionEngine:
    ...
    def use_list(self, output_dir: str, metric_output: bool = False, xml_output: bool = False,
                 min_confidence: float = 0.0, image_ext: str = "jpg", image_dir=None, annotation_dir=None):

        # images = glob.glob(f"{image_dir}{dirsep}*{image_ext}")
        files = parse_test_txt_file("test.txt", image_dir, annotation_dir)
        pred_output_folder = ""
        xml_output_folder = ""
        img_output_folder = ""
        if xml_output:
...
def get_latest_caffemodel():
    ...
    snapshot_glob = glob.glob(os.path.join("snapshot", "*caffemodel"))
...
default_output_dir = f"test_{os.path.basename(default_model)}"

# Constructing Argument Parse to input from Command Line
ap = argparse.ArgumentParser()
ap.add_argument("-i", "--image_dir", default=None, required=False, help='Path to images')
ap.add_argument("-e", "--image_ext", default="jpg", required=False, help='Image extension')
ap.add_argument("-a", "--annotation_dir", default=None, required=False, help='Path to annotations')
...
output_path = os.path.abspath(os.path.join(FLAGS.output_dir, "results"))
absolute_path_groundtruth = os.path.abspath(os.path.join(FLAGS.output_dir, "groundtruths"))
absolute_path_detection = os.path.abspath(os.path.join(FLAGS.output_dir, "predictions"))
